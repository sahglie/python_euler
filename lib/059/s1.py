...
def solve(cipher):
    for key in itertools.permutations(xrange(97, 123), 3):
        solution = []
        for i,item in enumerate(cipher):
            solution.append(key[i%3] ^ item)
        if " the " in "".join(map(chr, solution)):
            return sum(solution)

if __name__ == "__main__":
    with open("cipher.txt") as fd:
        cipher = map(int, fd.read().strip().split(","))
        print solve(cipher)
